...
public class SIG0Signer {
  ...
  public SIG0Signer(int algorithm, PrivateKey privateKey, Name name, PublicKey publicKey) {
    ...
    this.privateKey = privateKey;
    this.name = name;
    Record rec = KEYConverter.buildRecord(name, Type.KEY, DClass.IN, 0, KEYRecord.OWNER_USER, KEYRecord.PROTOCOL_ANY, algorithm, publicKey);
  ...
  public void apply(Message m, byte[] old) throws IOException, SignatureException, InvalidKeyException, NoSuchAlgorithmException {
    ...
    SIGRecord tmpsig = new SIGRecord(Name.root, DClass.ANY, 0, 0, algorithm, 0, timeExpires, timeSigned, footprint, name, null);
    ...
    /*
     * RSA signatures are already in correct format, but Java DSA routines use
     * ASN.1; convert this to SIG format.
     */
    if (algorithm == DNSSEC.DSA) {
    ...
    SIGRecord sig = new SIGRecord(Name.root, DClass.ANY, 0, 0, algorithm, 0, timeExpires, timeSigned, footprint, name, signature);
