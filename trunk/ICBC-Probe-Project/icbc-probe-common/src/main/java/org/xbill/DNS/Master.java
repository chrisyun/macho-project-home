...
public class Master {
   ...
   * @throws IOException
   *           The master file could not be read, or was syntactically invalid.
   */
    ...
    while (true) {
      ...
      if (token.type == Tokenizer.WHITESPACE) {
        ...
        if (s.equalsIgnoreCase("$ORIGIN")) {
          ...
          /*
           * If we continued, we wouldn't be looking in the new file. Recursing
           * works better.
           */
          return nextRecord();
  ...
  /**
   * Specifies whether $GENERATE statements should be expanded. Whether expanded
   * or not, the specifications for generated records are available by calling
   * {@link #generators}. This must be called before a $GENERATE statement is
   * seen during iteration to have an effect.
   */
