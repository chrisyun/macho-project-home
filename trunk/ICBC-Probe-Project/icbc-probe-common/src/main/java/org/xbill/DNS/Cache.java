...
public class Cache {
  ...
  private static class CacheMap extends LinkedHashMap {
    ...
    void setMaxSize(int maxsize) {
      /*
       * Note that this doesn't shrink the size of the map if the maximum size
       * is lowered, but it should shrink as entries expire.
       */
      this.maxsize = maxsize;
    }
   ...
   * @param o
   *          The source of the record (this could be a Message, for example)
   * @see Record
   */
   ...
   * @param soa
   *          The SOA record to add to the negative cache entry, or null. The
   *          negative cache ttl is derived from the SOA.
   * @param cred
  ...
  protected synchronized SetResponse lookup(Name name, int type, int minCred) {
    ...
    for (tlabels = labels; tlabels >= 1; tlabels--) {
      ...
      /*
       * If this is the name, look for the actual type or a CNAME. Otherwise,
       * look for a DNAME.
       */
      if (isExact) {
  ...
  /**
   * Sets the maximum length of time that a negative response will be stored in
   * this Cache. A negative value disables this feature (that is, sets no
   * limit).
   */
  ...
  /**
   * Gets the maximum length of time that a negative response will be stored in
   * this Cache. A negative value indicates no limit.
   */
   ...
   * A negative value disables this feature (that is, sets no limit).
   */
  ...
  /**
   * Gets the current number of entries in the Cache, where an entry consists of
   * all records with a specific Name.
   */
  public int getSize() {
    return data.size();
  }
  ...
  /**
   * Gets the maximum number of entries in the Cache, where an entry consists of
   * all records with a specific Name. A negative value is treated as an
   * infinite limit.
   */
  ...
  /**
   * Sets the maximum number of entries in the Cache, where an entry consists of
   * all records with a specific Name. A negative value is treated as an
   * infinite limit.
   * 
   * Note that setting this to a value lower than the current number of entries
   * will not cause the Cache to shrink immediately.
   * 
