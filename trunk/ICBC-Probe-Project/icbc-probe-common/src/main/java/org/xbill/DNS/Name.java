...
public class Name implements Comparable, Serializable {
  ...
  /*
   * Effectively an 8 byte array, where the low order byte stores the number of
   * labels and the 7 higher order bytes store per-label offsets.
   */
   ...
   * 
   * @param in
   *          A stream containing the DNS message which is currently positioned
   *          at the start of the name to be read.
   */
   ...
   * 
   * @param n
   *          The label to be converted to a (printable) String. The first label
   *          is 0.
   */
