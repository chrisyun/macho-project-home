...
public class ExtendedResolver implements Resolver {
  ...
  private static class Resolution implements ResolverListener {
    ...
    public Resolution(ExtendedResolver eres, Message query) {
      ...
      if (eres.loadBalance) {
        ...
        /*
         * Note: this is not synchronized, since the worst thing that can happen
    ...
    public Message start() throws IOException {
      try {
         ...
         * Otherwise, we'll get an exception and continue. It would be easier to
         * call send(0), but this avoids a thread creation. If and when
         * SimpleResolver.sendAsync() can be made to not create a thread, this
         * could be changed.
         */
        sent[0]++;
        outstanding++;
        ...
        /*
         * This will either cause more queries to be sent asynchronously or will
         * set the 'done' flag.
         */
    ...
    /*
     * Receive a response. If the resolution hasn't been completed, either wake
    ...
    /*
     * Receive an exception. If the resolution has been completed, do nothing.
    ...
    public void handleException(Object id, Exception e) {
      ...
      synchronized (this) {
        ...
        if (e instanceof InterruptedIOException) {
          ...
          /*
           * Problem with the response; don't resend on the same socket.
           */
          thrown = e;
        }
        if (done)
   ...
   * 
   * @param servers
   *          An array of server names for which SimpleResolver contexts should
   *          be initialized.
   * @see SimpleResolver
   * @exception UnknownHostException
   *              Failure occured initializing SimpleResolvers
