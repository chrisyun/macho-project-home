...
public class DiscoveryServiceHandler {
     ...
     */
    protected DiscoveryServiceHandler(Element config, Map<String, IdPSiteSet> federations, Map<String, Plugin> plugins,
        ...
        for (int i = 0; i < list.getLength(); i++) {
            ...
            if (siteset == null) {
                LOG.error("Handler " + location + ": could not find metadata for <Federation> with identifier "
                        + attribute + ".");
                throw new ShibbolethConfigurationException("Handler " + location
                        + ": could not find metadata for  <Federation> identifier " + attribute + ".");
            }
        ...
        for (int i = 0; i < list.getLength(); i++) {
            ...
            if (plugin == null) {
                ...
                throw new ShibbolethConfigurationException("Handler " + location
                        + ": could not find plugin for identifier " + attribute);
            }
     ...
     */
    private void handleSelection(HttpServletRequest req, HttpServletResponse res) throws WayfRequestHandled,
            WayfException {
     ...
     * <ul>
     * <li>If a "return" parameter is present we check in the metadata for spoofing and then set up from there</li>
     * <li>If "returnID" is specified we get this from the metadata</li>
     * <li>If nothing is provided we get the default from the metadata (if provided)</li>
     * <li>Otherwise we whine</li>
     * </ul>
     * 
    ...
    /**
     * Helper function to see whether the provided endpoint in the metadata matches the provided return URL in the
     * request.
     * 
     * @param discovery
     * @param providedName
     ...
     */
    private void handleLookup(HttpServletRequest req, HttpServletResponse res, String searchName) throws WayfException,
            WayfRequestHandled {
                ...
                if (metadataProvider.containsSP(providerId) || !config.getLookupSp()) {
                    ...
                    for (int i = 0; i < thePlugins.size(); i++) {
                        ...
                        if (searchResults == null) {
                            ...
                            // This was a search
                            ctx[i] =
                                    plugin.lookup(req, res, metadataProvider.paramFor(plugin), theseSites, ctx[i],
                                            hintList);
                        } else {
                            ctx[i] =
                                    plugin.search(req, res, metadataProvider.paramFor(plugin), searchName, theseSites,
                                            ctx[i], searchResults, hintList);
                        }
                    }
    ...
    /**
     * Check for SAML2 endpoints.<br />
     * We have been approached with a DS message. An SP may not be announcing it SAML2-ness in the metadata This allows
     * us to check.
     * 
    ...
    private void checkForSAML2EndPoints(EntityDescriptor sp) {
        ...
        if (null == ssoDescriptor) {
            LOG.warn("Service Provider " + sp.getEntityID()
                    + " requested DiscoveryProtocol, but lacks SAML2 SPSSODescriptor with protocol "
                    + XMLConstants.SAML2_PROTOCOL);
            return;
        }
    }
