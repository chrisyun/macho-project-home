//设定editor的绝对地址
//设定img 为一个真实的图片地址:img.src = '/static/logo.gif'; line:7033  不设也可以，图片错误就没地址；设置了可以调试
//修改上传文件的提交地址 line:390
//editor 页面 提交前增加： onsubmit="javascript:editor.entyArea.value = editor.entyBody.innerHTML;"
// line:8349 加 editor.entyBody.innerHTML = editor.entyArea.value; 为了编辑的时候预添加内容
// line:8343 8323 filter() 不注释的话 会导致 表单元素转换不过来


var sinaeditorpath = '/statics/sinaeditor/';
var up_domain = '/up.union.com/';
...
SinaEditor.CONF.STYLELOC = '/statics/sinaeditor/style/';
	...
	'<div id="#{useClient}" class="urlPath">',
		'<div id="#{clientView}">',
			...
			'<div id="#{clientUploadDiv}" class="clientUploadDiv" >',
				...
				//TODO 这里需要配置上传的地址。
				'<form target="#{clientIframe}"  id="#{clientForm}" action="'+up_domain+'?op=uploadfilelist&c=file&get=2" method="POST" enctype="multipart/form-data">',
					'<input type="file" name="file" class="imgFile" id="#{clientFile}">',
		...
		if(SinaEditor.env.$IE) {
			var sc = SinaEditor.util.dom.createDom('script', {
	            ...
	            'attributes': {
	                ...
	                'src':sinaeditorpath+'ierange-m2.js?'+(+new Date())
	            }
...
SinaEditor.plugins.add('showSource', function(args){
    ...
    editor.operation.swapData = function(toArea){
        ...
        if (toArea) {
            ...
            //filter();alert(editor.entyBody.innerHTML);
            //if(args.formatter) {
    ...
    return {
        'initialize': function(){
			...
			editor.entyBody.innerHTML = editor.entyArea.value;
